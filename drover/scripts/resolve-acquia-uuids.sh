...
#
# DROVER_ACLI overrides the acli binary (for tests).

set -uo pipefail

ACLI="${DROVER_ACLI:-acli}" python3 -c '
import json, os, subprocess, sys

acli = os.environ["ACLI"]
...
apps = run_json([acli, "api:applications:list"]) or []
app_by_site = {}
cache_envs_per_app = {}

def envs_for(app_uuid):
    if app_uuid not in cache_envs_per_app:
        cache_envs_per_app[app_uuid] = run_json(
