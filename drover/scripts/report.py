...
def _append_supplementary_detail(lines: list[str], agg: dict) -> None:
    """Append the non-ranking php-error detail table when data exists."""
    groups = agg.get("supplementary_groups", []) or []
    if not groups:
        return
    ...
    lines.append(
        ...
        "detail for the issues already identified above._"
    )
    lines.append("")
    ...
    for group in groups[:10]:
        ...
        summary = _truncate(group.get("summary") or "", 100)
        summary = summary.replace("|", "\\|")
        lines.append(
            f"| {_severity_chip(severity)} | "
            f"{_fmt_int(group.get('count', 0))} | {summary} |"
        )
    lines.append("")
    ...
    *,
    ...
    if total == 0:
        ...
        if groups:
            ...
            lines.append("")
            lines.append(
                f"The largest strong-signal source was the "
                f"**{top.get('channel') or 'unspecified'}** channel "
                f"({_fmt_int(top['count'])} events, "
            ...
            lines.append("")
            lines.append(
                "No watchdog or apache issues were available to drive "
                "the ranked storyline."
            )
    lines.append("")
...
def generate_report(
    ...
    *,
    ...
    strong_groups, supplementary_groups = _aggregate.split_by_signal_tier(
        agg.get("groups", []) or []
    )
    agg = {
        **agg,
    ...
    summary = {
        ...
        "supplementary_groups_total": len(supplementary_groups),
        "coverage_pct": (
            100.0 * coverage["present_days"] / coverage["expected_days"]
            if coverage["expected_days"] else 100.0
        ),
...
def generate_data(
    ...
    *,
    ...
    Returns a dict with the schema:
      {
        drover_schema_version: 2,
        generated_at: ISO-8601,
        meta: {project, env, month, month_label, prior_month, from, to,
               types},
        coverage: {expected_days, present_days, coverage_pct,
                   missing_or_failed: [...]},
        totals: {events_total, groups_total, supplementary_groups_total,
                 by_severity, by_channel, by_day},
        groups: [...full fingerprint groups with optional .delta...],
        supplementary_groups: [...flat php-error detail groups...],
