---
name: changelog
description: Display ideate plugin changelog from CHANGELOG.md. Use when the user mentions "ideate" and wants to know what changed — e.g. "ideate changelog", "what's new in ideate", "what changed in ideate", "ideate release notes". Default (no args): full CHANGELOG. Optional args: "--latest" shows only the most recent version; "--since X.Y.Z" shows all versions after X.Y.Z.
allowed-tools: Bash
---

# ideate Release Notes

Read and output the ideate CHANGELOG.

## Procedure

```bash
bash "${CLAUDE_SKILL_DIR}/read-changelog.sh" "ideate" "${CLAUDE_PLUGIN_ROOT}" "${ARGUMENTS:-}"
```
