#!/usr/bin/env bats
# Tests for workshop/scripts/check-integration.sh circuit-breaker behavior.
...
setup() {
  ...
  mkdir -p "${HOME}/.claude/plugins/data/workshop"
  export REAL_CACHE_FILE="${HOME}/.claude/plugins/data/workshop/integration-cache.json"
...
_write_healthy_cache() {
  ...
  python3 -c "
import json, time, os
f = os.path.expanduser('~/.claude/plugins/data/workshop/integration-cache.json')
data = {}
if os.path.exists(f):
