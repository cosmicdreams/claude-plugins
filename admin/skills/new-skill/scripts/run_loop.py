...
and selecting the best description by held-out test score to prevent overfitting.
Uses claude -p (subscription auth) throughout — no API key needed.

Run from the skill directory:
  python3 -m scripts.run_loop --eval-set evals/trigger-evals.json --skill-path . --model claude-sonnet-4-6
"""
...
def run_loop(
    ...
    for iteration in range(1, max_iterations + 1):
        ...
        if verbose:
            ...
            print_stats("Train", train_results["results"], eval_elapsed)
            if test_summary:
                print_stats("Test ", test_results["results"], 0)
        ...
        t0 = time.time()
        # Hide test scores from the improvement step to prevent overfitting
        blinded_history = [
...
def main():
    ...
    parser.add_argument("--model", required=True, help="Model for claude -p calls")
    parser.add_argument("--verbose", action="store_true", help="Print progress to stderr")
    parser.add_argument("--results-dir", default=None, help="Save results JSON to a timestamped subdir here")
